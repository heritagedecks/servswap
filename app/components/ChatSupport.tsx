...
// This would be your knowledge base in a real implementation
// For now, we'll expand our simple responses object to include more detailed information
const knowledgeBase = {
  // General info
...
// Updated topic matches with account creation keywords
const topicMatches = {
  // Pricing related
...
const getAIResponse = (query: string): string => {
  ...
  if (topMatch && topMatch.score > 0.15) {
    console.log(`Matched "${query}" with ${topMatch.id} (score: ${topMatch.score.toFixed(2)})`);
...
export default function ChatSupport() {
  ...
  const [messages, setMessages] = useState<{ text: string; isUser: boolean }[]>([
    { text: "Hi there! I'm ServSwap's AI assistant. How can I help you today?", isUser: false },
  ]);
  const [inputValue, setInputValue] = useState('');
  const [isTyping, setIsTyping] = useState(false);
  const [useApi, setUseApi] = useState(true); // Control whether to use API or client-side
  const messagesEndRef = useRef<HTMLDivElement>(null);
  const inputRef = useRef<HTMLInputElement>(null);

  // Check if the current page should show the chat support
  const shouldShowChat = pathname === '/' || pathname === '/support' || pathname === '/pricing';
  ...
  // If not on a page where chat should be shown, don't render anything
  if (!shouldShowChat) {
    return null;
  }
    ...
    try {
      ...
      if (useApi) {
        try {
          ...
          // Add AI response with slight delay to simulate typing
          setTimeout(() => {
            setMessages(prev => [...prev, { text: data.message, isUser: false }]);
            setIsTyping(false);
          }, 800 + Math.random() * 800); // Random delay between 800-1600ms
        } catch (error) {
          console.warn('API error, falling back to client-side:', error);
          ...
          // Switch to client-side mode after API failure
          setUseApi(false);
          
          // Add a system message about offline mode
          setTimeout(() => {
            setMessages(prev => [...prev, { 
              text: "I'm now operating in offline mode. Some advanced features may be limited.",
              isUser: false 
            }]);
          }, 1800);
        }
      } 
      // Use client-side AI for offline support
      else {
      ...
      setMessages(prev => [...prev, { 
        text: "I'm having trouble processing your request. Please try again or contact support@servswap.com for assistance.", 
        isUser: false 
      }]);
      setIsTyping(false);
    }
  ...
  const suggestedQuestions = [
    ...
    "What's the process for joining ServSwap?",
    "How much does a subscription cost?",
    "How do I get verified on ServSwap?",
  ...
  const handleSuggestedQuestion = async (question: string) => {
    ...
    try {
      ...
      const response = await fetch('/api/chat', {
        ...
        body: JSON.stringify({
          message: question,
          conversationHistory: messages, // Include full context
        }),
      });
      ...
      const fallbackResponse = getAIResponse(question);
      setMessages(prev => [...prev, { text: fallbackResponse, isUser: false }]);
    } finally {
      setIsTyping(false);
    }
  ...
  // Function to attempt to reconnect to the API
  const handleReconnect = async () => {
    try {
      // Simple ping to the API to check if it's available
      const response = await fetch('/api/chat', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
      ...
      if (response.ok) {
        setUseApi(true);
        setMessages(prev => [...prev, { 
          text: "I'm back online with full capabilities!",
          isUser: false 
        }]);
        return true;
      }
  ...
  return (
    <>
      ...
      <button
        onClick={toggleChat}
        className="fixed bottom-6 right-6 p-4 bg-gradient-to-r from-indigo-600 to-purple-600 text-white rounded-full shadow-lg hover:shadow-xl transition-all duration-300 z-50 flex items-center justify-center"
        ...
        {isOpen ? (
          ...
          <div className="relative">
            ...
            {useApi ? (
              ...
              <div className="absolute -top-1 -right-1 h-3 w-3 bg-yellow-400 rounded-full border-2 border-indigo-600"></div>
            )}
          </div>
        )}
      </button>
      ...
      {/* Chat window */}
      {isOpen && (
        <div className="fixed bottom-24 right-6 w-80 sm:w-96 h-96 bg-white rounded-xl shadow-2xl z-50 flex flex-col overflow-hidden border border-gray-200">
          {/* Chat header */}
          <div className="bg-gradient-to-r from-indigo-600 to-purple-600 p-4 text-white flex justify-between items-center">
            <div className="flex items-center">
              <Bot className="h-5 w-5 mr-2" />
              <h3 className="font-medium">ServSwap Assistant</h3>
              <div className="flex items-center ml-2 text-xs bg-white/10 px-2 py-0.5 rounded-full">
                <div className={`h-2 w-2 rounded-full mr-1 ${useApi ? 'bg-green-400 animate-pulse' : 'bg-yellow-400'}`}></div>
                <span>{useApi ? 'Online' : 'Offline'}</span>
              </div>
            </div>
            {!useApi && (
              <button 
                onClick={handleReconnect}
                className="text-xs bg-white/20 hover:bg-white/30 text-white px-2 py-1 rounded flex items-center"
                title="Try to reconnect to online mode"
              >
                <span className="mr-1">Reconnect</span>
              </button>
            )}
          ...
          <div className="flex-1 overflow-y-auto p-4 bg-gray-50">
            ...
            {isTyping && (
              <div className="mb-3 mr-auto max-w-[80%]">
                <div className="p-3 rounded-lg bg-white text-gray-800 border border-gray-200 rounded-bl-none flex items-center">
            ...
            {/* Suggested questions (visible only at the start) */}
            {messages.length === 1 && (
              <div className="mt-4">
                <p className="text-sm text-gray-500 mb-2">Here are some questions you might have:</p>
          ...
          <div className="p-3 border-t border-gray-200 bg-white">
            <div className="flex items-center">
              <input
                ...
                className="flex-1 py-2 px-3 border border-gray-300 rounded-l-lg focus:ring-indigo-500 focus:border-indigo-500 outline-none"
                disabled={isTyping}
              />
              <button
